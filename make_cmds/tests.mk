...
test-coverage: clean ## Run tests with coverage report
	...
	uv run pytest -c=config/pytest.ini --cov=src/meshunwrap --cov-report=term-missing --cov-report=html tests/

.PHONY: test-coverage-report
test-coverage-report: clean ## Run tests with coverage report (no fail-under; for CI/reporting)
	@$(call i, Running tests with coverage)
	uv run pytest -c=config/pytest.ini --cov=src/meshunwrap --cov-report=term-missing --cov-report=html --cov-fail-under=0 tests/
