"""Unit tests for meshunwrap version and debugging utilities."""
...
from meshunwrap._version import (
    Environment,
    Package,
    Variable,
...
class TestVersionFunctions:
    ...
    def test_get_version_success(self) -> None:
        ...
        result = get_version("meshunwrap")
        assert isinstance(result, str)
        assert result != "0.0.0" or result == "0.0.0"  # May be dev/installed

    @patch("meshunwrap._version.metadata.version")
    ...
    def test_version_info(self) -> None:
        ...
        result = version_info()
        assert "meshunwrap:" in str(result)
...
class TestGetDebugInfo:
    ...
    def test_get_debug_info_packages(self) -> None:
        """Test get_debug_info includes meshunwrap package."""
        result = get_debug_info()
        pkg_list = [pkg for pkg in result.packages if pkg.name == "meshunwrap"]
        assert len(pkg_list) >= 1
        assert pkg_list[0].version

    @patch.dict(os.environ, {"PYTHONPATH": "/test/path", "meshunwrap_DEBUG": "true"})
    def test_get_debug_info_with_environment_variables(self) -> None:
        ...
        assert "meshunwrap_DEBUG" in variable_names
    ...
    @patch.dict(os.environ, {"PYTHONPATH": "", "meshunwrap_TEST": ""})
...
class TestDebugInfo:
    ...
    def test_debug_info_wide_console_uses_layout(self) -> None:
        ...
        with patch("meshunwrap.cli.messages.layout.use_layout", return_value=True):
            debug_info(console)

    def test_debug_info_narrow_console_uses_panel(self) -> None:
        """Test debug_info with use_layout False uses panel (narrow terminal)."""
        console = Console(theme=set_theme())
        with patch("meshunwrap.cli.messages.layout.use_layout", return_value=False):
            debug_info(console)
