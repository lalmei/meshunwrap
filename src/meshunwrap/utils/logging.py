"""Logging utilities for meshunwrap CLI application."""
...
def _set_up_logger(
    ...
    *,
    ...
    if log_level is None:
        log_level = int(os.getenv("_MESHUNWRAP_LOG_LEVEL", DEBUG))
    else:
...
def get_logger_console(
    ...
    Args:
        name (str, optional): _description_. Defaults to "meshunwrap".
    ...
    """
    if log_level is None:
        log_level = int(os.getenv("_MESHUNWRAP_LOG_LEVEL", INFO))
    else:
        # Handle string log levels and invalid values gracefully
        try:
            if isinstance(log_level, str):
