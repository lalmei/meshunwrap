...
def get_version(dist: str = "meshunwrap") -> str:
...
def version_info() -> Text:
    ...
    """
    version = get_version()
    return Text.assemble(("meshunwrap: ", "peach"), (f"{version}", "bold"))
...
def get_debug_info() -> Environment:
    ...
    variables = [
        "PYTHONPATH",
        *[var for var in os.environ if var.startswith("meshunwrap")],
    ]
    return Environment(
...
def debug_info(console: Console | None = None) -> None:
    ...
    env = get_debug_info()
    from meshunwrap.cli.messages.layout import use_layout  # noqa: PLC0415 - deferred to avoid circular import
