...
def _register_commands(app: Typer, path: Path | None = None) -> None:
    ...
    for module_name in command_modules:
        ...
        try:
            ...
            module = importlib.import_module(f"meshunwrap.cli.commands.{module_name}")
            ...
            app.add_typer(sub_command, name=command_name)
            registered_commands.add(command_name)
            logger.debug(f"Registered command '{command_name}' from submodule '{module_name}'")
